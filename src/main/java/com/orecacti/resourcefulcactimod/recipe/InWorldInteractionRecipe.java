...
public class InWorldInteractionRecipe implements Recipe<RecipeInput> {
    ...
    public static class Serializer implements RecipeSerializer<InWorldInteractionRecipe> {
        ...
        private static final MapCodec<InWorldInteractionRecipe> CODEC = RecordCodecBuilder
                ...
                }, DataResult::success).forGetter(recipe -> recipe.recipeItems)).apply(builder, InWorldInteractionRecipe::new));
        public static final StreamCodec<RegistryFriendlyByteBuf, InWorldInteractionRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

        @Override
        public MapCodec<InWorldInteractionRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, InWorldInteractionRecipe> streamCodec() {
            return STREAM_CODEC;
        }

        private static InWorldInteractionRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
            ...
            return new InWorldInteractionRecipe(ItemStack.STREAM_CODEC.decode(buf), inputs);
        }

        private static void toNetwork(RegistryFriendlyByteBuf buf, InWorldInteractionRecipe recipe) {
